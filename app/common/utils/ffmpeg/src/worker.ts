...
import type {
  ...
  ExitCode,
  FSNode,
  FileData, FFMessageGetBasicParams, FFMessageKeyFrameList, FFMessageVideoBasicParams, FFMessageFrameList,
} from "./types";
...
const load = async ({
  ...
  const ffprobeWorkerURL = _ffprobeWorkerURL
    ? _ffprobeWorkerURL
    : _ffprobeCoreURL.replace(/.js$/g, ".worker.js");
  try {
