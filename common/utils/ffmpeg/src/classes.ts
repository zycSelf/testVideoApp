...
export class FFmpeg {
  ...
  public writeFileFFprobe = (path: string, data: FileData): Promise<OK> => {
    const trans: Transferable[] = [];
    if (data instanceof Uint8Array) {
      trans.push(data.buffer);
    }
  ...
  public deleteFFprobeFile = (path: string): Promise<OK> =>
    this.#send({
  ...
  public deleteFFprobeDir = (path: string): Promise<OK> =>
    this.#send({
      type: FFMessageType.DELETE_FFPROBE_FILE,
      data: { path },
    }) as Promise<OK>;
